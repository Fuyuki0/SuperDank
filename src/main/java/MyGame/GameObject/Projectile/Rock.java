...
public class Rock extends Projectile {
    ...
    public void updateProj(double deltaTime, World world) {
        ...
        for (int j = 0; j < world.getEnemies().size(); j++) {
            ...
            if ((distance < rockRadius * rockRadius) && !world.getPlayer().isJumping() && enemy.getRockHitTime() > 0.3 / (Math.max(1, world.getOrbitRock().getRockCount()) / 2) / ( 1 + world.getOrbitRock().getBonusAttackSpeed() / 100)) {
    ...
    public void draw(GraphicsContext gc, double cameraPosX, double cameraPosY, double screenWidth, double screenHeight, double margin, GameEngine engine) {
        ...
        double sourceY = 0;
        gc.save();
        gc.setGlobalAlpha(Math.max(this.spawn(), 90));
        double currentAngle = orbitRock.getAngle() + this.getCurrentRelativeAngle();
        double currentRadius = orbitRock.getRADIUS() * this.spawn();
